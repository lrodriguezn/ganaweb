...
beforeAll(async () => {
  ...
  // Reference the shared canonical permission (animales:ver) instead of inserting a
  // duplicate: uq_usuarios_permisos UNIQUE (modulo, accion) rejects a second row with the
  // same natural key on a seeded database. Mirrors animal-listado-postgres.test.ts.
  await execute(sql`
