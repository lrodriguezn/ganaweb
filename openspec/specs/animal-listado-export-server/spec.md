# Animal Listado Export Server Specification

## Purpose

Define #111's server-side contract for exporting the online animal list. Excel (XLSX), CSV, and landscape PDF are generated entirely on the server for the complete filtered set — not a single page — reusing the #107 filters and order. Generation is online-only; the server is authoritative for scope/column rules, config-driven operational limits, spreadsheet-injection neutralization, RBAC re-validation with per-finca isolation, and a sanitized error contract.
