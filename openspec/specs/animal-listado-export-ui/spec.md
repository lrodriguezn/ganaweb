# Animal Listado Export UI Specification

## Purpose

Define #111's export UI for the online animal list: the scope/format dialog with the PDF 36-column warning, the fetch→blob→download transport, the non-destructive error and retry contract, and the visual RBAC gate. The UI consumes the serialized #107 query read-only and never renders the artifact inline; the server remains authoritative for authorization and generation.
