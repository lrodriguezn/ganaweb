...
| Accent normalization | Persisted columns; collation/translation; PostgreSQL extension | Migration `0003` installs `unaccent` in `public`; queries explicitly call `public.unaccent` and `pg_catalog.lower`. This is the smallest complete correction and leaves write paths untouched. Persisted normalized columns require separate approval if capability validation fails. |
| Pattern semantics | Wildcard input; literal input | Escape `!` as `!!`, `%` as `!%`, and `_` as `!_`, wrap the result in `%`, bind the complete pattern, and emit `ESCAPE '!'`. No request identifier is interpolated. |
| Capability failure | Retain case-only search; fail rollout | Fail before rollout. Never silently retain `lower(...) LIKE`; validate the actual production listing role independently of migration-role success. |
...
The query predicates `animales.finca_id = fincaId AND activo = 1`; combines validated column predicates with AND and `q` as accent/case-insensitive OR over code, name, ear tag, and RFID. `escapeLikeLiteral` and `normalizedContains` produce `public.unaccent(pg_catalog.lower(column)) LIKE public.unaccent(pg_catalog.lower($boundPattern)) ESCAPE '!'`. The same shared WHERE feeds page rows and filtered count; `totalSinFiltro` remains finca-wide. Catalogs are left joins; missing optional relations map to `null`. `config_key_values` joins `opcion = 'tipo_ingreso'` and key/value mapping; unknown non-null IDs map to `{ id, label: "Desconocido (<id>)" }`. Dates are serialized as ISO dates, age is calculated at request time to one decimal, and boolean fields are always non-null.
...
| `packages/db/migrations/0003_animal_list_unaccent.sql` and journal | Create/Modify | Idempotently provision qualified `public.unaccent` and assert callable/EXECUTE capability without editing `0002`. |
...
| Accent search | all four `q` and ten `contains` fields in both accent directions; `%`, `_`, `!`, SQL-like literals; finca isolation; exact counters; tied pages 1/2 | Controlled lower-only RED, then PostgreSQL GREEN using real `public.unaccent`; 26 equal-sort rows prove page-size-25 stability and exact union. |
| Migration capability | fresh and already-provisioned migration chains; callable and EXECUTE checks | Disposable PostgreSQL 17 validates `to_regprocedure`, `has_function_privilege`, and `public.unaccent('Árbol')`; production listing role remains a deployment gate. |
...
Generate and review additive Drizzle migrations, apply them with `pnpm --filter @ganaweb/db migrate`, and validate `public.unaccent` before enabling accent predicates. If creation or invocation fails, stop rollout; an unusable extension may remain, but case-only behavior is not accepted. Roll back code independently; never edit an applied migration or drop a shared extension without a reviewed forward migration. Existing btree indexes narrow finca/active rows but do not prove leading-wildcard `unaccent` performance or §11 acceptance. Legacy behavior remains untouched.
...
- [ ] Validate the actual production listing credential can invoke `public.unaccent(text)` if it differs from the migration role; local superuser evidence is not production-role proof.
