...
async function seedSistema(db: ReturnType<typeof createClient>) {
  ...
  const MODULOS: Record<string, string[]> = {
    animales:               ['ver', 'crear', 'editar', 'inactivar', 'eliminar'], // eliminar = borrado FÍSICO (CA-DEL, solo sin eventos)
  ...
  const MATRIZ: Record<string, string[]> = {
    // Administrador: TODO (incluye animales:eliminar — borrado físico)
    'rol-admin': todosPermisos.map((p) => p.id),
    // Mayordomo: opera el día a día; no configura, no anula movimientos
    'rol-mayordomo': [
      // Mayordomo NO recibe animales:eliminar — su vía es el autoservicio
      // de 24h sobre sus propios registros (CA-DEL-008, validado en servidor)
