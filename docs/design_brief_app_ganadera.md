...
## Mapa de pantallas del `.op` (v1.2 — 21 pantallas × 10 temas = 210)

El archivo `ganaweb-diseno.op` tiene 10 páginas (5 estilos × claro/oscuro:
GanaWeb/Campo, Dark Mode, Moderna, Índigo, Cielo, Grafito y sus · Dark).
Cada página contiene las MISMAS 21 pantallas — al construir, la estructura
sale de cualquier página y el tema sale de los tokens (`ganaweb-estilos.md`).

| # | Pantalla | Ruta TanStack | Notas |
|---|---|---|---|
| 01 | Dashboard · Desktop | `/fincas/$fincaId` | Métricas + acción requerida |
| 02 | Inicio · Mobile | `/fincas/$fincaId` | Familia moderna: hero bento |
| 03 | Animales · Mobile | `/fincas/$fincaId/animales` | Cards + selección múltiple |
| 04 | Ficha Animal · Mobile | `/animales/$animalId` | Tabs + timeline |
| 05 | Registrar Vacuna (drawer) | — (EventDrawer) | Contrato de formularios de evento |
| 06 | Sanidad · Mobile | `/fincas/$fincaId/sanidad` | Calendario + stock |
| 07 | Reporte · Desktop | `/fincas/$fincaId/reportes` | KPIs + gráficos + tabla |
| 08 | Login · Mobile | `/login` | "Mantener sesión" marcado |
| 09 | Registro · Mobile | `/registro` | Código de finca opcional (PE-007) |
| 10 | Login · Desktop | `/login` | Banda brand-panel |
| 11 | Configuración · Desktop | `/fincas/$fincaId/configuracion` | Grid de maestros (RBAC) |
| 12 | Maestro Veterinarios | `/configuracion/veterinarios` | Patrón CRUD de maestro |
| 13 | Configuración · Mobile | ídem 11 | |
| 14 | Selector Finca · Desktop | — (FincaSwitcher) | Estados sync por finca |
| 15 | Selector Finca · Mobile | — (Drawer) | Advertencia de pendientes |
| 16 | Apariencia · Mobile | — ("Más → Apariencia") | EstiloSelector 5 estilos + modo |
| 17 | Menú Avatar · Desktop | — (dropdown del shell) | Apariencia personal, no admin |
| 18 | Animales · Desktop | `/fincas/$fincaId/animales` | Tabla 44px + filtros + selección |
| 19 | Ficha Animal · Desktop | `/animales/$animalId` | 2 columnas: resumen + timeline |
| 20 | Nuevo Animal · Desktop | `/animales/nuevo` y `/animales/$id/editar` | MISMO formulario ambos modos; en edición el código se bloquea si hay eventos (RN-001) |
| 21 | Nuevo Animal · Mobile | ídem 20 | Fullscreen, footer pegajoso, pills para sexo/origen |

Pendientes conocidos del mapa (no diseñados aún): formularios de evento
restantes del EventDrawer (peso, palpación, servicio, producción — usar 05
como contrato), Dashboard desktop en tratamiento bento para la familia
moderna, y estados de tabla (vacío / cargando / sin resultados).
