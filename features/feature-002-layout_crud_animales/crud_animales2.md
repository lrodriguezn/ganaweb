# GanaWeb — Especificación CRUD Animales (v1.6)
...
| 3 | **ORIGEN** | Pills Nacido/Comprado + panel condicional (§3.5.2.2) | pills 260px + panel `1fr 1fr` |
| 4 | **▸ UBICACIÓN** (colapsable) | Potrero, Sector, Lote, Grupo | `1fr 1fr` — **2 por fila** (CA-UI-019) |
...
#### 3.5.2.2 Sección ORIGEN — panel condicional (rediseño v1.6)

> Defecto observado: al conmutar de "Nacido en finca" a "Comprado" los
> campos del nuevo modo se **superponen** a los del anterior. Es un bug de
> montaje (los del modo saliente no se desmontan, CA-UI-007) Y un diseño
> frágil: campos sueltos que aparecen y desaparecen en el flujo del
> formulario producen saltos de layout. Se corrigen ambas cosas.

- **CA-UI-021 · Panel contenedor** — Los condicionales de Origen NO son
  campos sueltos bajo las pills: viven dentro de un **panel único**
  (`rounded-card bg-muted/40 p-4`, un solo hijo del DOM) que se renderiza
  con `key={modoOrigen}` para forzar el remontaje completo al conmutar.
  Un solo nodo entra y sale: es imposible que coexistan dos modos.
- **CA-UI-022 · Contenido del panel** — Grilla `1fr 1fr` dentro del panel:
  - *Nacido en finca*: Madre · Padre.
  - *Comprado*: Fecha de compra · Precio de compra (fila 1);
    Peso de compra · Lugar de compra (fila 2).
- **CA-UI-023 · Altura estable** — El panel reserva `min-height` igual a
  la del modo más alto (2 filas), de modo que conmutar no desplace el
  contenido posterior del formulario.
- **CA-UI-024 · Descarte de valores** — Al conmutar, los valores del modo
  abandonado se limpian del estado del formulario (no viajan ocultos al
  submit) — reafirma CA-UI-007 con el remontaje de CA-UI-021 como
  mecanismo.

#### 3.5.2.1 Sección UBICACIÓN (colapsable, v1.6)

- **CA-UI-019** — La ubicación es un bloque **colapsable** con exactamente
  **2 dropdowns por fila** (`1fr 1fr`): fila 1 Potrero + Sector, fila 2
  Lote + Grupo. La fila de 4 columnas queda derogada: a 720px cada select
  quedaba de ~170px, insuficiente para nombres como "Potrero La Represa".
  Siguen siendo 4 controles separados e independientes (CA-UI-003).
- **CA-UI-020** — Comportamiento del colapsable de ubicación: cerrado por
  defecto al CREAR; abierto automáticamente al EDITAR si algún campo tiene
  valor. El encabezado resume lo seleccionado cuando está cerrado
  ("▸ Ubicación · POT-3 · Lote 4") o muestra "▸ Ubicación · sin asignar".
  Aplican igual CA-UI-010 (un error interno abre el bloque y enfoca).
...
- **CA-UI-025 — Sexo se mantiene como DROPDOWN también en mobile**
  (mismo control que desktop). No se sustituye por pills, segmented
  control ni radio buttons: el catálogo tiene 3 opciones (Macho, Hembra,
  Pajuela) y "Pajuela" no debe quedar escondida tras un control de dos
  estados. Regla explícita para impedir que una futura iteración lo
  "optimice" a pills.
- **Origen** sí usa **pills segmentadas** a ancho completo (son 2 opciones
  excluyentes que gobiernan un panel condicional; ver CA-UI-021).
...
6. Revisión de estilos (v1.5): cero hex literales, cero `dark:`, cero
   ...
   temas (CA-UI-016..018).
7. E2E v1.6: (a) conmutar Origen 5 veces seguidas no deja campos
   superpuestos ni valores del modo anterior en el submit (CA-UI-021/024);
   (b) el bloque Ubicación abre/cierra y muestra 2 selects por fila
   (CA-UI-019); (c) en mobile, Sexo renderiza como dropdown con las 3
   opciones del catálogo (CA-UI-025).
8. E2E de UI del formulario (v1.3): Sexo/Raza/Color/Calidad renderizan
